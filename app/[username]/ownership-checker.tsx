...
export function OwnershipChecker({
    profileUserId,
    username,
    profile,
    initialLinks,
    isTransparent
}: OwnershipCheckerProps) {
    const { user, isInitialized } = useAuthStore()
    const [isOwner, setIsOwner] = useState(false)

    useEffect(() => {
        // Only check ownership after auth is initialized
        if (isInitialized) {
            const ownershipStatus = user?.id === profileUserId
            setIsOwner(ownershipStatus)
            console.log('[OwnershipChecker] Ownership check:', {
                userId: user?.id,
                profileUserId,
                isOwner: ownershipStatus
            })
        }
    }, [user, profileUserId, isInitialized])
