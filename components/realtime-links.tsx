...
function TwitchLink({ link, username, iconStyle, mainTwitchChannel, activeTheme }: { link: SocialLink; username: string; iconStyle: IconStyle; mainTwitchChannel: string; activeTheme?: ColorScheme }) {
            ...
            >
                ...
                >
                    <Twitch
                        ...
                        style={getIconStyle('twitch', iconStyle, activeTheme)}
                    />
                ...
                {isLive && (
                    <AnimatePresence>
                        {isExpanded && (
                            ...
                            >
                                <iframe
                                    src={`https://player.twitch.tv/?channel=${username}&parent=${typeof window !== 'undefined' ? window.location.hostname : ''}`}
                                    frameBorder="0"
                                    allowFullScreen
...
export function RealtimeLinks({ userId, initialLinks, isOwner, iconStyle, mainTwitchChannel, activeTheme }: RealtimeLinksProps) {
    const {
        links,
        isUpdating,
        lastUpdateType,
                ...
                <AnimatePresence mode="popLayout">
                    {links.map((link, index) => {
                        ...
                        if (link.platform.toLowerCase() === 'twitch') {
                            ...
                            // Only treat as main if username matches mainTwitchChannel (case-insensitive)
                            const isMain = mainTwitchChannel && username.toLowerCase() === mainTwitchChannel.toLowerCase();
                            try {
                                return (
                                    <motion.div
                                        key={link.id}
                                    ...
                                    >
                                        <TwitchLink link={link} username={username} iconStyle={(iconStyle as IconStyle) || 'colored'} mainTwitchChannel={isMain ? mainTwitchChannel : ''} activeTheme={activeTheme} />
                                    </motion.div>
                                );
                            } catch (error) {
                    ...
                    `
                                ...
                                >
                                    ...
                                    <div className="relative flex items-center justify-center gap-2 sm:gap-3">
                                        <Icon
                                            className="w-4 h-4 sm:w-6 sm:h-6"
                                            style={getIconStyle(link.platform, (iconStyle as IconStyle) || 'colored', activeTheme)}
                                        />
                                        <span className="font-medium text-sm sm:text-base" style={{ color: `rgb(var(--theme-foreground))` }}>
                                            {link.title || link.platform}
                                        </span>
